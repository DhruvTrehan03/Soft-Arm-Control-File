...
# *********     SMS_STS Controller      *********
#
# A comprehensive controller for SMS_STS servos with the following features:
# - Ping/scan servos
# - Change servo IDs
...
class SMS_STSController:
    def __init__(self, port_name: str = None, baudrate: int = 115200):
        """
        Initialize the SMS_STS controller
    ...
    def set_zero(self, servo_id) -> bool:
        ...
        STS: write 128 to TORQUE_ENABLE (calibration mode).
        ...
        for sid in servo_ids:
            scs_comm_result, scs_error = self.packetHandler.write1ByteTxRx(sid, SMS_STS_TORQUE_ENABLE, 128)
            if scs_comm_result != COMM_SUCCESS:
                ...
                print(f"[ID:{sid:03d}] Set zero succeeded (STS TORQUE_ENABLE=128)")
                success_count += 1
    ...
    def set_torque(self, servo_id, torque) -> bool:
        """
        Set target torque (PWM) for a servo
        
        Args:
            servo_id: ID of the servo (int) or list of servo IDs
            torque: Target torque value(s) (PWM control)
        ...
        for t in torque_values:
            if t < 0:
                converted_torque_values.append(abs(t) + 1024)
            else:
        ...
        if len(servo_ids) == 1:
            ...
            scs_comm_result, scs_error = self.packetHandler.write2ByteTxRx(sid, SMS_STS_GOAL_TIME_L, converted_torque)
            if scs_comm_result != COMM_SUCCESS:
                print(f"Failed to set torque (PWM) for servo {sid}: {self.packetHandler.getTxRxResult(scs_comm_result)}")
                return False
            elif scs_error != 0:
                print(f"Error setting torque (PWM) for servo {sid}: {self.packetHandler.getRxPacketError(scs_error)}")
                return False
            else:
                return True
        else:
            ...
            if len(set(converted_torque_values)) == 1:
                ...
                converted_torque = converted_torque_values[0]
                # Check if parameters have changed
                if self._check_sync_write_params(servo_ids, SMS_STS_GOAL_TIME_L, converted_torque, 2):
                    # Create sync write group for torque (PWM) (2 bytes at address 44)
                    groupSyncWrite = self._create_sync_write_group(SMS_STS_GOAL_TIME_L, 2)
    ...
    def set_torque_limit(self, servo_id, torque_limit: int) -> bool:
        """
        Set torque limit for a servo (SMS_STS uses PWM control for torque limits)
        
        Args:
            servo_id: ID of the servo (int) or list of servo IDs
            torque_limit: Torque limit (0 to 1000) - converted to PWM control
        ...
        if len(servo_ids) == 1:
            ...
            if scs_comm_result != COMM_SUCCESS:
                print(f"Failed to set torque limit (PWM) for servo {sid}: {self.packetHandler.getTxRxResult(scs_comm_result)}")
                return False
            elif scs_error != 0:
                print(f"Error setting torque limit (PWM) for servo {sid}: {self.packetHandler.getRxPacketError(scs_error)}")
                return False
            else:
                return True
        else:
    ...
    def sync_read_posspeedload(self, servo_ids: List[int]) -> Dict[int, Tuple[int, int, int]]:
        ...
        """
        return self.sync_read_parameters(servo_ids, SMS_STS_PRESENT_POSITION_L, 6, "position/speed/load")
        ...
        for servo_id in servo_ids:
            ...
            if data_result:
                if data_length == 1:
                    ...
                    speed = groupSyncRead.getData(servo_id, SMS_STS_PRESENT_SPEED_L, 2)
                    load = groupSyncRead.getData(servo_id, SMS_STS_PRESENT_LOAD_L, 2)
    ...
    def sync_write_positions(self, servo_positions: Dict[int, int], 
                           ...
                           acc: Union[int, List[int]] = 50) -> bool:
        """
        Synchronously write position commands to multiple servos (SMS_STS version without torque)
        ...
        if len(speed) != num_servos or len(acc) != num_servos:
            print(f"Parameter array lengths must match number of servos. Got: speed={len(speed)}, acc={len(acc)}, servos={num_servos}")
            return False
        
        # Clear previous parameters
        self.packetHandler.groupSyncWrite.clearParam()
        
        # Add parameters for all servos using SDK's SyncWritePosEx (SMS_STS version without torque)
        for i, (servo_id, position) in enumerate(servo_positions.items()):
            result = self.packetHandler.SyncWritePosEx(servo_id, position, speed[i], acc[i])
            if not result:
                print(f"Failed to add parameter for servo {servo_id}")
                return False
    ...
    def individual_write_position(self, servo_id: int, position: int, 
                                speed: int = 100, acc: int = 50) -> bool:
        """
        Write position command to a single servo (SMS_STS version without torque)
    ...
    def read_servo_state(self, servo_id) -> Optional[Union[Tuple[int, int, int, int], Dict[int, Tuple[int, int, int, int]]]]:
        ...
        # Handle single ID or list of IDs
        if isinstance(servo_id, int):
            # Single servo - read position, speed, load, and current individually
            sid = servo_id
            
            # Read position
            position, comm_result, error = self.packetHandler.ReadPos(sid)
            if comm_result != COMM_SUCCESS:
                print(f"Failed to read position from servo {sid}: {self.packetHandler.getTxRxResult(comm_result)}")
                return None
            elif error != 0:
                print(f"Error reading position from servo {sid}: {self.packetHandler.getRxPacketError(error)}")
                return None
            
            # Read speed
            speed, comm_result, error = self.packetHandler.ReadSpeed(sid)
            if comm_result != COMM_SUCCESS:
                print(f"Failed to read speed from servo {sid}: {self.packetHandler.getTxRxResult(comm_result)}")
                return None
            elif error != 0:
                print(f"Error reading speed from servo {sid}: {self.packetHandler.getRxPacketError(error)}")
                return None
            
            # Read load (using sync read for single servo)
            load_results = self.sync_read_parameters([sid], SMS_STS_PRESENT_LOAD_L, 2, "load")
            if sid not in load_results:
                print(f"Failed to read load from servo {sid}")
                return None
            load = load_results[sid]
            
            # Read current from address 69 (right after load read)
            current_results = self.sync_read_parameters([sid], SMS_STS_PRESENT_CURRENT_L, 2, "current")
            ...
            current_results = self.sync_read_parameters(servo_id, SMS_STS_PRESENT_CURRENT_L, 2, "current")
            # Combine results
            combined_results = {}
            for sid in servo_id:
...
if __name__ == "__main__":
    ...
    if controller.connect():
        ...
        if found_servos:
            # Enable torque for all found servos
            controller.enable_torque(found_servos)
            
            # Set all servos to center position
            center_positions = {servo_id: 2048 for servo_id in found_servos}
            controller.sync_write_positions(center_positions, speed=100, acc=50)
            
            # Read servo states
            states = controller.read_servo_state(found_servos)
            print(f"Servo states: {states}")
