...
class MotorControllerNode(Node):
    def __init__(self, name_prefix: str = '', debug_mode: bool = False, serial_port: str = None):
        ...
        try:
            self.controller = SMS_STSController(self.serial_port, 115200)
    ...
    def handle_position_command(self, parameters):
        ...
        for idx, (motor_id, position_deg) in enumerate(zip(motors, positions)):
            if motor_id in self.motor_ids:
                ...
                # Store speed and acceleration for this position command
                self.position_move_speed[motor_index] = speed[idx]
                self.acceleration_commands[motor_index] = acc[idx]
                #print(f"Stored position command for motor {motor_id}: {position_deg}°")
        ...
        if servo_positions:
            ...
            result = self.controller.sync_write_positions(servo_positions, individual_speeds, individual_accs)
            if result:
    ...
    def send_stored_commands(self):
        ...
        if position_motors:
            self.execute_position_commands(position_motors, position_values, position_speeds, position_accs)
    ...
    def execute_position_commands(self, motors, positions, speeds=None, accelerations=None):
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if data and len(data) == 4:
                                ...
                                # Debug: Log raw load values for all motors
                                # self.get_logger().info(f"Motor {motor_id} raw data - pos: {position}, speed: {speed}, load: {load}, current: {current}")
