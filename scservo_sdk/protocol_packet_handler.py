...
class protocol_packet_handler(object):
    ...
    def read6ByteTx(self, scs_id, address):
        return self.readTx(scs_id, address, 6)

    def read6ByteRx(self, scs_id):
        data, result, error = self.readRx(scs_id, 6)
        if result == COMM_SUCCESS:
            # Combine 6 bytes into a 48-bit value (3 words)
            data_read = (self.scs_makeword(data[0], data[1]) |
                        (self.scs_makeword(data[2], data[3]) << 16) |
                        (self.scs_makeword(data[4], data[5]) << 32))
        else:
            data_read = 0
        return data_read, result, error
        
    def read6ByteTxRx(self, scs_id, address):
        data, result, error = self.readTxRx(scs_id, address, 6)
        if result == COMM_SUCCESS:
            # Combine 6 bytes into a 48-bit value (3 words)
            data_read = (self.scs_makeword(data[0], data[1]) |
                        (self.scs_makeword(data[2], data[3]) << 16) |
