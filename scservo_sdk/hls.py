...
class hls(protocol_packet_handler):
    ...
    def ReadPosSpeedLoad(self, scs_id):
        scs_present_position_speed_load, scs_comm_result, scs_error = self.read6ByteTxRx(scs_id, HLS_PRESENT_POSITION_L)
        scs_present_position = self.scs_loword(scs_present_position_speed_load)
        scs_present_speed_load = self.scs_hiword(scs_present_position_speed_load)
        scs_present_speed = self.scs_loword(scs_present_speed_load)
        scs_present_load = self.scs_hiword(scs_present_speed_load)
        return self.scs_tohost(scs_present_position, 15), self.scs_tohost(scs_present_speed, 15), self.scs_tohost(scs_present_load, 15), scs_comm_result, scs_error
